...
export default function ProductDetails({ product }: Props) {
  ...
  const dynamicAttrs = Object.entries(DYNAMIC_ATTR_LABELS)
    ...
    .map(([key, label]) => ({
      label,
      value: Array.isArray(product[key]) ? (product[key] as string[]).join(', ') : String(product[key]),
    }))
  ...
  return (
    <Grid container spacing={4}>
      ...
      <Grid item xs={12} md={6}>
        ...
        <Chip
          label={product.stock > 0 ? `${product.stock} disponibles` : 'Agotado'}
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          <Stack direction="row" alignItems="center" sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
            ...
            <IconButton onClick={() => setCantidad((c) => Math.min(product.stock, c + 1))} disabled={cantidad >= product.stock}>
