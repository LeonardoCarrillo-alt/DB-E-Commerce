    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <FormControl fullWidth size="small">
        ...
        <Select
          value={filters.marca ?? ''}
          label="Marca"
          onChange={(e) => onChange({ marca: e.target.value || undefined })}
        >
          <MenuItem value="">Todas</MenuItem>
          {BRANDS.map((brand) => (
        ...
        <>
          ...
          <Box>
            ...
            <Stack direction="row" flexWrap="wrap" gap={0.5}>
              {filters.categoria && (
                <Chip label={filters.categoria} size="small" onDelete={() => onChange({ categoria: undefined })} color="primary" variant="outlined" />
              )}
              {filters.marca && (
                <Chip label={filters.marca} size="small" onDelete={() => onChange({ marca: undefined })} color="secondary" variant="outlined" />
              )}
            </Stack>
          </Box>
        </>
