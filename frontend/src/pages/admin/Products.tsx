...
export default function AdminProducts() {
  ...
  const { data, isLoading } = useProducts({ page, limit: 12, busqueda: search || undefined })
  ...
  const handleSubmit = async (values: ProductFormValues, extraAttrs: Record<string, unknown>) => {
    try {
      if (editingProduct) {
        ...
        await createProduct.mutateAsync({ ...values, ...extraAttrs, activo: true } as Partial<Product>)
