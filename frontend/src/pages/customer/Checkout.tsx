...
import { Container, Typography, Grid, Card, CardContent, Box, Alert, Stepper, Step, StepLabel } from '@mui/material'
...
export default function Checkout() {
  ...
  const handleSubmit = async (data: CheckoutFormValues) => {
    setError(null)
    try {
      const payload = orderService.buildOrderPayload(items, data)
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Stepper activeStep={1} sx={{ mb: 4, display: { xs: 'none', sm: 'flex' } }}>
        <Step completed><StepLabel>Carrito</StepLabel></Step>
        <Step active><StepLabel>Información de envío y pago</StepLabel></Step>
